...
**Availability:** `set_vars` requires nono **v0.63.0 or later**. On earlier versions the profile fails to load with an `unknown field set_vars` error — upgrade nono rather than treating it as a typo.
